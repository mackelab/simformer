...
def main():
    ...
    # %%
    # Reverse SDE drift
    def drift_ode(t, x, x_o,node_ids=node_id, condition_mask=np.array(condition_mask), edge_mask=edge_mask1, score_fn = model_fn):
    ...
    def sample_post(key, x_o):
        thetas =  sample_fn(key, (500,), node_id, condition_mask, x_o, edge_mask=edge_mask1)[:, :2]
    ...
    def log_probs_post(thetas, x_o):
        def f(x_T):
            y = odeint(lambda t,x: drift_ode(t,x,x_o, edge_mask=edge_mask1), x_T, jnp.linspace(1e-5,1, 100)[::-1], method="rk3")[-1]
    ...
    jnp.save("log_probs_q_vec_partial1_posterior.npy", log_probs_q_vec)
    jnp.save("log_probs_true_vec_partial1_posterior.npy", log_probs_true_vec)
